 ...
 */
export async function savePage(scanResult, embeddingResult) {
    try {
        // Save page data
        ...
        if (pageError || !pageData) {
            throw new Error(`Failed to save page: ${pageError?.message || "No data returned"}`);
        }
        // Save embedding data
        ...
        throw new Error(`Failed to save page and embedding: ${error.message}`);
    }
}
/**
...
export async function getPageByURL(url) {
    try {
        ...
        throw new Error(`Failed to retrieve page by URL: ${error.message}`);
    }
}
/**
...
export async function getPageById(id) {
    try {
        ...
        throw new Error(`Failed to retrieve page by ID: ${error.message}`);
    }
}
/**
 ...
 */
export async function markPageDecayed(id) {
    try {
        const { data, error } = await supabase
            .from("pages")
            ...
            .single();
        if (error || !data) {
            throw new Error(`Failed to mark page as decayed: ${error?.message || "No data returned"}`);
        }
        return data;
    }
    catch (error) {
        throw new Error(`Failed to mark page as decayed: ${error.message}`);
    }
}
/**
 ...
 */
export async function saveReconstruction(pageId, reconstruction) {
    try {
        const { data, error } = await supabase
        ...
        throw new Error(`Failed to save reconstruction: ${error.message}`);
    }
}
/**
 ...
 */
function generateHints(scan) {
    const hints = [];
    if (scan.keywords)
        hints.push(`Keywords: ${scan.keywords}`);
    if (scan.headings)
        hints.push(`Main topics: ${scan.headings}`);
 ...
 */
export async function savePageWithExtras(scan, emb, extras) {
 ...
 */
export async function addPendingScan(url, source = "discovery", priority = 0, metadata = {}) {
    const { data, error } = await supabase
        .from("pending_scan")
        ...
        .single();
    if (error)
        throw new Error(`Failed to add pending scan: ${error.message}`);
    return data;
}
export async function getPendingBatch(batchSize = 20) {
    const { data, error } = await supabase
        .from("pending_scan")
        .select("*")
        .eq("status", "pending")
        ...
        .limit(batchSize);
    if (error)
        throw new Error(`Failed to fetch pending batch: ${error.message}`);
    return data || [];
}
export async function markPendingProcessing(id) {
    ...
    const { error: updateError } = await supabase
        ...
        .eq("id", id);
    if (updateError) {
        throw new Error(`Failed to mark pending processing: ${updateError.message}`);
    }
}
export async function markPendingDone(id) {
    const { error } = await supabase
        .from("pending_scan")
        .update({ status: "done" })
        .eq("id", id);
    if (error)
        throw new Error(`Failed to mark pending done: ${error.message}`);
}
export async function markPendingFailed(id, reason) {
    ...
    const { error } = await supabase
        ...
        .eq("id", id);
    if (error)
        throw new Error(`Failed to mark pending failed: ${error.message}`);
}
