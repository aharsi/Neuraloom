...
import { getPendingBatch, markPendingProcessing, markPendingDone, markPendingFailed, } from "../supabaseClient.js";
 ...
 * - mark done / failed
 */
async function processPending(pending) {
    const id = pending.id;
    try {
        await markPendingProcessing(id);
    }
    catch (err) {
        console.error("Failed to mark processing", id, err.message);
    ...
    try {
        ...
        await pRetry(async () => {
            ...
            retries: 2,
            onFailedAttempt: (err) => console.warn(`Attempt failed for ${pending.url}: ${err.message}`),
        });
        await markPendingDone(id);
        console.log(`Processed and saved: ${pending.url}`);
    }
    catch (err) {
        console.error(`Processing failed for ${pending.url}:`, err.message);
        try {
            await markPendingFailed(id, err.message);
        }
        catch (e) {
            console.error("Failed to mark pending failed:", e.message);
        }
    }
}
/**
