 ...
 */
function weightedAverage(embeddings: number[][], weights: number[]): number[] {
  if (
    embeddings.length === 0 ||
...
export async function generateEmbedding(
  ...
  try {
    ...
    const metaEmbeddings: Record<string, number[]> = {};
    const fieldEmbeddings: number[][] = [];
    const weights: number[] = [];

    validFields.forEach((field, i) => {
      if (embeddings[i]) {
        // Type guard to ensure embeddings[i] is defined
        metaEmbeddings[field.name] = embeddings[i] as number[];
        fieldEmbeddings.push(embeddings[i] as number[]);
