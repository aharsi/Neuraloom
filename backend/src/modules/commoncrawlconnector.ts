...
export async function fetchLatestCommonCrawlIndex(): Promise<string | null> {
  try {
    ...
    console.error(
      "Failed to fetch Common Crawl collinfo:",
...
export async function queryCommonCrawlIndex(
  ...
  try {
    ...
    const url = `https://index.commoncrawl.org/${encodeURIComponent(
      indexId
    )}-index?url=*${encodeURIComponent(pattern)}*&output=json`;
    ...
    const entries: { url: string; title?: string; source: string }[] = [];
    ...
    console.error("CommonCrawl index query failed:", (err as Error).message);
    return [];
  }
}
