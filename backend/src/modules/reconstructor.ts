 ...
 */
async function getEmbeddingAndMetaByPageId(pageId: string): Promise<{
  ...
  try {
    ...
    if (embeddingError) {
      throw new Error(
        `Failed to retrieve embedding: ${embeddingError.message}`
      );
    }
    ...
    const { data: pageData, error: pageError } = await supabase
      .from("pages")
      .select(
        "title, meta_description, headings, intro_paragraphs, keywords, embedding_meta"
      )
      .eq("id", pageId)
      .single();
    ...
    throw new Error(
      `Failed to retrieve embedding and metadata for page ${pageId}: ${
...
export async function reconstructPage(
  ...
  try {
    ...
    let metaEmbeddings: Record<string, number[]> | undefined;
    ...
    let reconstructionRecord: ReconstructionRecord;
    if (input.pageId) {
      ...
      reconstructionRecord = data as ReconstructionRecord;
    } else {
    ...
    throw new Error(`Failed to reconstruct page: ${(error as Error).message}`);
  }
}
