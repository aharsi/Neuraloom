...
export async function logFailure(url: string, reason: string) {
  try {
    ...
    console.error("Failed to write system log:", (err as Error).message);
  }
}
...
export async function checkFailureSpikes(windowMinutes = 5, threshold = 30) {
  ...
  if (count >= threshold) {
    console.warn(
      `High error spike detected: ${count} errors in last ${windowMinutes} minutes`
    );
