...
// Add at top
const LAST_N_HOURS = 96; // No. of hours
 ...
 */
export async function connectorArxiv(
  rssUrl = "https://export.arxiv.org/rss/cs"
  ...
  try {
    ...
    console.error("arXiv connector failed:", (err as Error).message);
    return [];
  }
}
 ...
 */
export async function connectorOpenAlex(
  cursor?: string,
  perPage = 50
): Promise<{ url: string; title?: string; source: string }[]> {
  try {
    ...
    console.error("OpenAlex connector failed:", (err as Error).message);
    return [];
  }
}
 ...
 */
export async function connectorCrossRef(
  rows = 50
): Promise<{ url: string; title?: string; source: string }[]> {
  try {
    ...
    console.error("CrossRef connector failed:", (err as Error).message);
    return [];
  }
}
 ...
 */
export async function connectorCommonCrawl(
  query = "education filetype:pdf"
): Promise<{ url: string; title?: string; source: string }[]> {
  try {
    const indexUrl = `${
      process.env.COMMONCRAWL_INDEX_API || "https://index.commoncrawl.org/"
    }CC-MAIN-2024-22-index?url=*${encodeURIComponent(
      "education"
    )}*&output=json`;
    ...
    return lines
      ...
      .filter(Boolean) as { url: string; title?: string; source: string }[];
  } catch (err) {
    console.error("CommonCrawl connector failed:", (err as Error).message);
    return [];
  }
}
...
export async function runDiscoveryCycle(): Promise<{
  ...
  const rawResults: { url: string; title?: string; source: string }[] = [];
    ...
    connectors.map((c) =>
      queue.add(async () => {
        try {
          ...
          console.error("Connector error:", (err as Error).message);
        }
      })
    )
  );
  ...
  for (const c of candidates.slice(0, DISCOVERY_BATCH)) {
    try {
      ...
      let pr = 0;
      if (c.url.includes("doi.org")) pr += 0.8;
      if (c.url.endsWith(".pdf")) pr += 0.5;
      if (c.source === "arxiv") pr += 0.4;

      await addPendingScan(c.url, c.source, pr, { title: c.title });
      added++;
    } catch (err) {
      console.error("❌ Failed to enqueue:", c.url, (err as Error).message);
      skipped++;
    }
  }
