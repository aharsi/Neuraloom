 ...
 */
export async function computeDecayProbability(
  url: string,
  ...
  try {
    ...
    try {
      const who = (await whois(host, { follow: 3, timeout: 10000 })) as any;
      ...
      console.warn("WHOIS lookup failed:", (err as Error).message);
    }

    let sslFactor = 0.5;
    try {
      const ssl = await (sslChecker as any)(host, {
        method: "GET",
        timeout: 10000,
      });
      if (ssl.validTo) {
    ...
    console.error("computeDecayProbability error:", (err as Error).message);
    return 0.5; // fallback
  }
}
